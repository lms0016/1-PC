...
void fcl_make_normal_req1 (ioreq_event *parent, int blkno) {  // fox
	...
	if(ln){ // hit case 
		...
		if (fcl_cache_mgr->cm_free != 0 && prize > minP) {  
			...
			if (prize > minPrizeNode->cn_prize) { // prize > minPrize
				ln = fcl_replace_cache( parent, blkno, NULL); 
				ASSERT( ln );
			} else {  
				goHDD = 1;
...
void _fcl_make_stage_req ( ioreq_event *parent, struct lru_node *ln, int list_index ) {
	fcl_generate_child_request ( parent, HDD, reverse_get_blk(SSD, ln->cn_ssd_blk),	READ, list_index++, 0);
...
void fcl_get_next_request ( int op_type ) {
	...
	if ( op_type == FCL_OPERATION_NORMAL ) {
		...
		req->prize = calculate_prize(req->blkno, baseprize, req->bcount, req->flags, req->inSSD );
...
void _fcl_request_arrive ( ioreq_event *parent, int op_type ) {
	ioreq_event *req;
	int i = 0, j = 0, hit = 0, last_hit;
	struct lru_node *ln;
	ioreq_event *tmp[1024];
	for (i=0; i<1024; i++) {
		tmp[i] = NULL;
	}
	//parent->bcount = 2048;
	...
	// fox: check request segment is at HDD or at SSD, and split request
	tmp[j] = (ioreq_event *) getfromextraq();
	tmp[j]->blkno = parent->blkno;
	tmp[j]->bcount = 0;
	tmp[j]->flags = parent->flags;
	tmp[j]->time = simtime;
	for ( i = parent->blkno; i < (parent->blkno + parent->bcount); i+=FCL_PAGE_SIZE) {
		ln = CACHE_PRESEARCH ( fcl_cache_mgr, i );
		if (ln) 
			hit = 1;
		else 
			hit = 0;
		if ( j == 0 ) {
			tmp[j]->bcount += FCL_PAGE_SIZE;
			tmp[j]->inSSD = hit;
			last_hit = hit;
	...
	if (j==0) {
		...
		for (i=0; i<j; i++) {
			fcl_parent_init ( tmp[i] );
			tmp[i]->tempint1 = op_type;
			fcl_add_new_request( tmp[i], op_type );
